...
dataset = load_dataset("parquet", data_files=f"{parquet_folder}/*.parquet", cache_dir="data/cache", features=datasets.Features({
    ...
    "recaption": datasets.Value("string"),
    "recaption_short": datasets.Value("string"),
...
num_shards_per_push = 20 # You want this relatively high so you don't run into rate limits but not too high so it can't push the shards
